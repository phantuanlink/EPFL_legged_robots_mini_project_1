...
def evaluate_jumping(trial: Trial, simulator: QuadSimulator) -> float:
    ...
    if jumpmode_ == JumpMode.FORWARD:
        ...
        objective_value += np.abs(simulator.get_base_position()[0]) + 0.5 * np.abs(simulator.get_base_position()[2]) # Example: maximize forward distance
    elif jumpmode_ == JumpMode.SIDE:
        print(f"Furthest jump value: Y={simulator.get_base_position()[1]}, Z={simulator.get_base_position()[2]}")
        objective_value += np.abs(simulator.get_base_position()[1]) + 0.3 * np.abs(simulator.get_base_position()[2]) # Example: maximize sideway distance
        objective_value -= np.abs(simulator.get_base_position()[0]) * 0.3 # penalize for forward movement
